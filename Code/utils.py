...
class Statistics:
    ...
    def plot1D_traj(self):
        ...
        for idx in range(3):
            ax[idx].plot(
                self.time_list[:T], self.traj_true[:T, idx], "-.", label="true"
            )
            ax[idx].plot(
                self.time_list[:T], self.traj_pred[:T, idx], "-.", label="pred"
            )
            ax[idx].legend()
            ax[idx].set_ylabel(self.axis_names[idx])
...
class Dynamics:
    ...
    def compute_equilibrum(self):
        ...
        jacob_system = lambda w: self.trained_model.jacobian(w).numpy()
        model_system = lambda w: self.trained_model(torch.tensor(w, dtype=torch.float)).detach().numpy()

        fix_point_pred = self.newton(model_system, jacob_system, init_pos)
