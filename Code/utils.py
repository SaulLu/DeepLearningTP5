...
class Statistics:
    def __init__(self, wandb_logger, delta_t=1e-2, ts_n=10000, fft_n=1000):
    ...
    def add_traj(self, traj_true, traj_pred, time_list, prefix=None):
        ...
        self.time_list = np.asarray(time_list)
        self.prefix = prefix
    ...
    def plot_corr(self):
        ...
        fig.set_figwidth(15)
        fig.set_figheight(15)
        x_corr = np.linspace(-T // 2, T // 2, T)
        for idx in range(3):
            corr_true = np.correlate(
                self.traj_true[:T, idx], self.traj_true[:T, idx], "same"
            )
            corr_pred = np.correlate(
                self.traj_pred[:T, idx], self.traj_pred[:T, idx], "same"
            )
            ax[idx].plot(x_corr, corr_true, "-.", label="true")
            ax[idx].plot(x_corr, corr_pred, "-.", label="pred")
            ax[idx].legend()
            ax[idx].set_ylabel(self.axis_names[idx])
    ...
    def plot_fft(self):
        ...
        for idx in range(3):
            ...
            fmax = 1/(len(spec_true) * self.delta_t)
            x_fft = np.linspace(-0.5, 0.5, len(spec_true))
            ax[idx].plot(x_fft, spec_true, "-.", label="true")
            ax[idx].plot(x_fft, spec_pred, "-.", label="pred")
            ax[idx].legend()
            ax[idx].set_ylabel(self.axis_names[idx])
...
class Dynamics:
    ...
    def compute_lyaponov(self):
        lyap_pred = self.lyapunov_exponent(self.traj_pred, self.trained_model.jacobian, mode="discrete")
        lyap_true = self.lyapunov_exponent(self.traj_true, self.true_model.jacobian, mode="continuous")
    ...
    def compute_equilibrum(self):
        ...
        f_system = (
            lambda w: (self.trained_model(torch.tensor(w, dtype=torch.float)).detach().numpy() - w)
            / self.delta_t
        )
